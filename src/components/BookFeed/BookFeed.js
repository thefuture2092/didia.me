...
const BookActionButtons = ({edge}) => {
  ...
  return (
    <CardActions>
      <CardActionButtons>
        {edge.node.html ? (
          <Link className={'mdc-button mdc-card__action mdc-card__action--button'} to={edge.node.fields.slug}>
            Lire mon résumé
          </Link>
        ) : (
          <></>
        )}
        {edge.node.frontmatter.amazonLink ? (
          <a
            className={'mdc-button mdc-card__action mdc-card__action--button'}
            href={edge.node.frontmatter.amazonLink}
            target="_blank"
...
const ReadingTime = ({edge}) =>
  edge.node.frontmatter.readingEnd ? (
    ...
    >
      {`Lu du ${moment(edge.node.frontmatter.readingStart).format('D MMMM YYYY')} au ${moment(
        edge.node.frontmatter.readingEnd
      ).format('D MMMM YYYY')}`}
    </time>
  ) : (
...
const CurrentBook = ({edge}) => (
  <Card className={classnames(styles['feed__item'], 'mdc-card')}>
    <div className={styles['feed__item-body']}>
      <Headline5 className={styles['feed__item-title']}>{edge.node.frontmatter.title}</Headline5>
    ...
    <CardPrimaryContent className={styles['feed__item-content']}>
      ...
      <div className={styles['feed__item-body']}>
        <Body2 className={styles['feed__item-description']}>{edge.node.frontmatter.description}</Body2>
...
const BookFeed = ({edges}) => [
  ...
  <div className={styles['feed']} key="feed">
    {edges.slice(1).map(edge => (
      <Book edge={edge} key={edge.node.fields.slug} />
    ))}
  </div>
];
